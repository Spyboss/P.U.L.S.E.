...
# Import the agent
try:
    from main import GeneralPulseAgent
    logger.info("Successfully imported GeneralPulseAgent")
except ImportError as e:
    logger.error(f"Failed to import GeneralPulseAgent: {str(e)}")
    sys.exit(1)

async def test_grok_api_with_agent():
    """Test using Grok through the agent's model_query_intent."""
    logger.info("Initializing GeneralPulseAgent...")
    agent = GeneralPulseAgent()
